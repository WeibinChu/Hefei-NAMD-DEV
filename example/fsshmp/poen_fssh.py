...
############################################################
nsw     = 2000        # useless if all_en.npy exist
nproc   = 2           #
prefix  = 'NAMD/run/' #
...
########################################
bmin     = 310    # start from 1
bmax     = 324    # start from 1
namdTime = 1000
potim    = 1.0
Eref     = 'EVBM' # 'ECBM' / 'EVBM'
inpFiles = glob('MPSHPROP.*') # MP
########################################
### po.npy: 3D 3*namdtime*nband+2, initime|average energy|bands
if not os.path.isfile('po.npy'):

    iniTimes = [int(F.split('.')[-1])-1 for F in inpFiles]

    dat = np.array([np.loadtxt(F) for F in inpFiles])
    dat = np.average(dat, axis=0) # ensemble average
    ...
    for start in iniTimes:
        ...
        En_t /= len(iniTimes) # time average energy

    for i in range(bmax - bmin + 1):
        Time[:,i] = np.arange(namdTime)

    # time, average energy, for plot
    # dat[:,1] -= EREF[Eref] # MP
    x, y = dat[:,:2].T 
...
kmap = ax.scatter(Time, En_t, c=Ci_t/2, # /2 MP
                  cmap='hot_r',
                  vmin=0,
                  vmax=1,
